...
- **`wgpl validate [--strict] [INTERFACE_NAME_OR_ID]`**: Dry-run integrity report (active peer collisions, pool fit, DNS, corrupt `expires_at`, invalid wire fields, **routing topology**). Errors exit 1; warnings exit 0. With `--strict`, warnings also exit 1 (useful for CI gating). Does not mutate state. JSON is a typed report (`status` + `issues` at the top level).
